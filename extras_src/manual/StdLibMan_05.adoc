...
// @FIXME: Add XRef:
Similarly, it is possible for the hero to sit or lie down on a SUPPORTER object (>lie on bed, >sit on chair), but the action is not successful by default and must be manually implemented by the author.
