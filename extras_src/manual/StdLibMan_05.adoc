...
You won't need to define the hero in your game at all if you're happy with the response "You notice nothing unusual about yourself." when the player types [.play]#&gt; _x me_# (= examine myself) and if no attributes are needed for the hero (such as IS (NOT) hungry, HAS strength 20, etc.).
...
If you wish to have the pieces of clothing worn by the hero listed at other verbs, like for example [.play]#&gt; _ x me_#, you should use the formulation LIST worn.:
