...
==== {asterisk}{asterisk}{asterisk} ORIGINAL EXAMPLE TEXT {asterisk}{asterisk}{asterisk}

[WARNING]
=============
What follows was the original text introducing the above example.
It was originally inside the `DARK_LOCATION` section, illustrating practical examples of situations in which is more convenient to use an unlit location instead.

Since the chapter on "`Dark Places`" has now been reorganized, the example was moved to the "`Unlit Location`" section, and the original text no longer applies to the context.

I need to find a way to adapt the original text to the new context, because its tips are still useful.
=============
...
// @EXTERN TRANSCRIPT: sewers.a3ADocLog
[example,role="gametranscript"]
================================================================================
...
We'll implement the forest location as a `DARK_SITE`, so it will have a `ground` and `sky`, and we'll start off the game with forest being lit, instead of the default dark state.
