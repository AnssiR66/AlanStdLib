...
When there's something on a SUPPORTER, a listing of it will appear in the room description and after "`>examine`", by default:
...
Note that the "`>examine`" command will list what is on the surface of a SUPPORTER, not what, if anything, is inside the SUPPORTER.
