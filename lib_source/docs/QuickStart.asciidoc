...
** Copying the `newgame.alan` template and `StdLib/` folder into the adventure work folder.
...
[CAUTION]
=====================================
*TBD!* We should mention the `newgame.alan` template here!
...
* [ ] Add to `newgame.alan` template region-tag comments so that we can selectively `include::` its various parts in this document.
=====================================

To facilitate the creation of new adventures using the ALAN StdLib we've included in the {ExtrasLink} folder of the library distribution the {newgame_alan} template file:

[source,alan,role=lib]
--------------------------------------
include::{ExtrasDistro}/newgame.alan[]
--------------------------------------

This template contains the minimum required code to setup an ALAN StdLib adventure which compiles to a working game (provided the library files are correctly setup and reachable to the template).
...
[CAUTION]
=====================================
*TBD!* Explain how to copy contents of `StdLib/` folder plus `newgame.alan` into source adventure working folder, then rename the boilerplate.

Then test that the boilerplate compiles.
=====================================



== Editing the Game Boilerplate

.EDITORS' NOTE
[CAUTION]
=====================================
*TBD!* In this chapter we'll go through the various sections of the boilerplate, and how to tweak it.
=====================================


=== The my_game Meta Instance
...
https://www.ifwiki.org/Zork_I
https://ifdb.org/viewgame?ifid=ZCODE-88-840726
////

// @TODO: Externalize sample adventure, so we can test it and get a real
//        transcript (use subfolder with '_' in name: '_quickstart/')

.Custom Banner Example: Zork I
[source,alan]
------------------------------------------------------------------------------
...
// @CHECK: There seems to be an error in the boilerplate:
//          * Missing '.' after "IsA location".

.Custom Starting Location: Zork I
[source,alan]
------------------------------------------------------------------------------
The house_west IsA site.
  Name 'West of House'.
...
You must then remember to also rename `MY_START_LOC` in the `Start` section, at the end of the template:
