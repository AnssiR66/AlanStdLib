...
** The old `worn` entity and the `wearing` set have been dropped, replaced by the `worn` (boolean) and `wearer` (reference) attributes: the former is now used to track if a clothing items is being donned by a character (hero or NPC), and the latter to quickly obtain a reference to its wearer (or the `nobody` dummy actor if unworn), e.g. in verbs CHECK failure responses when attempting to move around a worn item.
The `worn` and `wearer` attributes are defined on the `thing` class so that authors might also use them for wearables other than clothes (e.g. wearable devices, like VR headsets, etc.), by defining their own custom wearable classes, unconnected to the `clothing` class.
